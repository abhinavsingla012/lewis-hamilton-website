...
## Scroll-Bound Car: Drive → Park → Open — 2026-09-03 (supersedes the scrub/snap model)
- User rule: the car follows the scroll exactly and never moves on its own; pins dive to the car and STOP at the gate; the page opens only after one more scroll
- Engine (SpatialExperience `driveRef` + `park()` + `goTo({park})` + `intent(dir, animated)`): wheel/touch drive the car between the two neighbouring gates (`SCROLL_GAIN` 2 px/px, `TOUCH_GAIN` 2.2, `DRIVE_LERP` 12 smoothing only); the car stops where the scroll stops; reaching a gate parks it (`driveState` 'parked', hint pill "SCROLL DOWN/UP TO OPEN", 450ms cooldown); same-direction scroll opens the page (`driveState` 'none'), opposite-direction scroll drives back; scrolling from an open page closes it and drives away; hub is a gate too (reverse-drive from Legacy lands on the overview)
